...
int main(){
    ...
    w(t)
    {
        ...
        cin >> n ;
        vi vec ;
        loop(1,n+1,i)   vec.pb(i);
        ll st=1, end=n;

        while(1){
            auto itr = upper_bound(vec.begin(),vec.end(),(st+end)/2);
            if(itr == vec.end())    itr--;
            a = *itr ;
            out(a)
            cin >> c1;
            b = itr-vec.begin() ;
            auto itr1 = upper_bound(vec.begin(),vec.end(),(b+end)/2) ;
            if(*itr1 == a){
                itr1 = upper_bound(vec.begin(),vec.end(),(b+st)/2) ;
            }
            c = itr1-vec.begin() ;
            out(*itr1)
            cin >> c2;
